...
fn build_candidates(debian_dir: &Path, prefix: &str) -> Vec<CompletionItem> {
    ...
    let Some(files) = git_untracked(root, dir) else {
        return Vec::new();
    };

    let files = if files.len() == 1 && files[0] == dir {
        read_level(root, dir)
    } else {
        files
    };
...
fn git_untracked(root: &Path, dir: &str) -> Option<Vec<String>> {
    let mut command = std::process::Command::new("git");
    command.args(["ls-files", "--others", "--directory", "-z"]);
    if !dir.is_empty() {
        command.arg("--").arg(dir);
    }
...
/// One directory level below `root`.
fn read_level(root: &Path, dir: &str) -> Vec<String> {
...
mod tests {
    ...
    #[test]
    fn a_partly_tracked_directory_still_hides_its_tracked_files() {
        let dir = git_tree(&["build/Makefile"], &["build/out.o"]);
        let items = get_completions(
            "build/",
            Position::new(0, 6),
            Some(&dir.path().join("debian")),
        );
        assert_eq!(labels(&items), vec!["build/out.o".to_string()]);
    }
