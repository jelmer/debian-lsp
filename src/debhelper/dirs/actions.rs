...
/// Code actions for a debian/dirs file. The duplicate-entry quick fix is
/// shared by every debhelper helper; this is the dirs entry point.
...
mod tests {
    ...
    #[test]
    fn test_duplicate_dir_entry_is_removed() {
        let text = "usr/share/myapp\nusr/share/myapp\n";
        let idx = LineIndex::new(text);
        let src = Source::new(text, &idx);
        let uri: Uri = "file:///tmp/debian/dirs".parse().unwrap();
